...
def handle_connection(data, addr):
    ...
    if data["command"] == "create user":
        ...
        user_id = login(data["data"]["username"], data["data"]["password"], data["data"]["email"])
        send_response(addr, {"message": "You logged in.", "code": 201})
        print("You logged in.", user_id)
    elif data["command"] == "open room":
        user_info = data["user_info"]
        user_id = login(user_info["username"], user_info["password"], user_info["email"])
        if user_id is None:
            # Return error message to client
            send_response(addr, {"message": "Invalid username, password or mail.", "code": 401})
            return
...
def login(username, password, email):
    ...
    c = conn.cursor()
    c.execute("SELECT id, password FROM users WHERE username = ? AND email = ?", (username, email))
    user = c.fetchone()
    if user is not None:
